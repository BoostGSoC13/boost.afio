...
protected:
  ...
  async_io_handle(async_file_io_dispatcher_base *parent, std::shared_ptr<async_io_handle> _dirh, const filesystem::path &path, file_flags flags)
      : _parent(parent)
      , dirh(std::move(_dirh))
...
struct async_path_op_req
{
  ...
  */
  async_path_op_req(filesystem::path _path, file_flags _flags = file_flags::None)
      : path(_path)
      , flags(_flags)
  {
    if(!path.is_absolute())
  ...
  \param _flags The flags to be used.
  */
  async_path_op_req(async_io_op _precondition, filesystem::path _path, file_flags _flags = file_flags::None)
      : path(_path)
      , flags(_flags)
  ...
  \param _flags The flags to be used.
  */
  async_path_op_req(std::string _path, file_flags _flags = file_flags::None)
      : path(filesystem::absolute(filesystem::path(_path).make_preferred()))
      , flags(_flags)
  {
    _validate();
  }
  ...
  \param _flags The flags to be used.
  */
  async_path_op_req(async_io_op _precondition, std::string _path, file_flags _flags = file_flags::None)
      : path(filesystem::absolute(filesystem::path(_path).make_preferred()))
      , flags(_flags)
      , precondition(std::move(_precondition))
  {
  ...
  \param _flags The flags to be used.
  */
  async_path_op_req(const char *_path, file_flags _flags = file_flags::None)
      : path(filesystem::absolute(filesystem::path(_path).make_preferred()))
      , flags(_flags)
  {
    _validate();
  ...
  \param _flags The flags to be used.
  */
  async_path_op_req(async_io_op _precondition, const char *_path, file_flags _flags = file_flags::None)
      : path(filesystem::absolute(filesystem::path(_path).make_preferred()))
      , flags(_flags)
      , precondition(std::move(_precondition))
  {
...
struct async_enumerate_op_req
{
  ...
  */
  async_enumerate_op_req(async_io_op _precondition, size_t _maxitems = 2, bool _restart = true, filesystem::path _glob = filesystem::path(), metadata_flags _metadata = metadata_flags::None)
      : precondition(std::move(_precondition))
      , maxitems(_maxitems)
      , restart(_restart)
      , glob(std::move(_glob))
  ...
  \param _metadata The metadata to prefetch for each item enumerated. AFIO may fetch more metadata than requested if it is cost free.
  */
  async_enumerate_op_req(async_io_op _precondition, filesystem::path _glob, size_t _maxitems = 2, bool _restart = true, metadata_flags _metadata = metadata_flags::None)
      : precondition(std::move(_precondition))
      , maxitems(_maxitems)
      , restart(_restart)
      , glob(std::move(_glob))
  ...
  \param _glob An optional shell glob by which to filter the items returned. Done kernel side on Windows, user side on POSIX.
  */
  async_enumerate_op_req(async_io_op _precondition, metadata_flags _metadata, size_t _maxitems = 2, bool _restart = true, filesystem::path _glob = filesystem::path())
      : precondition(std::move(_precondition))
      , maxitems(_maxitems)
      , restart(_restart)
      , glob(std::move(_glob))
