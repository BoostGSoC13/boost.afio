...
#pragma warning(pop)
    ...
    DWORD len;
    char buffer[1024];
    if(!(len = FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_HMODULE, GetModuleHandleA("NTDLL.DLL"), code, 0, buffer, sizeof(buffer), 0)))
    {
      strcpy(buffer, "unknown error code");
      len = (DWORD) strlen(buffer);
    }
