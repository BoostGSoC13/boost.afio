...
namespace detail
{
  ...
  struct win_lock_file : public lock_file<win_actual_lock_file>
  {
    ...
    win_lock_file(async_io_handle *_h = nullptr)
        : lock_file<win_actual_lock_file>(_h)
        , ev(_h->parent()->threadsource()->io_service())
    {
      HANDLE evh;
  ...
  struct async_io_handle_windows : public async_io_handle
  {
    ...
    async_io_handle_windows(async_file_io_dispatcher_base *_parent, std::shared_ptr<async_io_handle> _dirh, const filesystem::path &path, file_flags flags)
  ...
  inline async_io_handle_windows::async_io_handle_windows(async_file_io_dispatcher_base *_parent, std::shared_ptr<async_io_handle> _dirh, const filesystem::path &path, file_flags flags, bool _SyncOnClose, HANDLE _h)
      : async_io_handle(_parent, std::move(_dirh), path, flags)
      , h(new asio::windows::random_access_handle(_parent->p->pool->io_service(), int_checkHandle(_h, path)))
      , myid(_h)
      , has_been_added(false)
...
#define FSCTL_SET_ZERO_DATA CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 50, METHOD_BUFFERED, FILE_WRITE_DATA)
    ...
    struct enumerate_state : std::enable_shared_from_this<enumerate_state>
    {
      ...
      enumerate_state(size_t _id, async_io_op _op, std::shared_ptr<promise<std::pair<std::vector<directory_entry>, bool>>> _ret, async_enumerate_op_req _req)
          : id(_id)
          , op(std::move(_op))
          , ret(std::move(_ret))
          , req(std::move(_req))
...
#define FSCTL_QUERY_ALLOCATED_RANGES CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 51, METHOD_NEITHER, FILE_READ_DATA)
  ...
  public:
    async_file_io_dispatcher_windows(std::shared_ptr<thread_source> threadpool, file_flags flagsforce, file_flags flagsmask)
  ...
  struct win_actual_lock_file : public actual_lock_file
  {
    ...
    win_actual_lock_file(filesystem::path p)
