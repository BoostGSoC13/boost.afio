...
namespace detail
{
  ...
  struct async_io_handle_windows : public async_io_handle
  {
    ...
    async_io_handle_windows(async_file_io_dispatcher_base *_parent, std::shared_ptr<async_io_handle> _dirh, const filesystem::path &path, file_flags flags) : async_io_handle(_parent, std::move(_dirh), path, flags), myid(nullptr), has_been_added(false), SyncOnClose(false), mapaddr(nullptr) {}
    ...
    BOOST_AFIO_HEADERS_ONLY_VIRTUAL_SPEC void close() override final { int_close(); }
    ...
    ~async_io_handle_windows() { int_close(); }
    BOOST_AFIO_HEADERS_ONLY_VIRTUAL_SPEC directory_entry direntry(metadata_flags wanted) const override final
    {
      ...
      {
        stat.st_ino = fai.InternalInformation.IndexNumber.QuadPart;
      }
      if(!!(wanted & metadata_flags::type))
      {
        stat.st_type = windows_nt_kernel::to_st_type(fai.BasicInformation.FileAttributes);
      }
      if(!!(wanted & metadata_flags::nlink))
      {
        stat.st_nlink = (int16_t) fai.StandardInformation.NumberOfLinks;
      }
      if(!!(wanted & metadata_flags::atim))
      {
        stat.st_atim = to_timepoint(fai.BasicInformation.LastAccessTime);
      }
      if(!!(wanted & metadata_flags::mtim))
      {
        stat.st_mtim = to_timepoint(fai.BasicInformation.LastWriteTime);
      }
      if(!!(wanted & metadata_flags::ctim))
      {
        stat.st_ctim = to_timepoint(fai.BasicInformation.ChangeTime);
      }
      if(!!(wanted & metadata_flags::size))
      {
        stat.st_size = fai.StandardInformation.EndOfFile.QuadPart;
      }
      if(!!(wanted & metadata_flags::allocated))
      {
        stat.st_allocated = fai.StandardInformation.AllocationSize.QuadPart;
      }
      if(!!(wanted & metadata_flags::blocks))
      {
        stat.st_blocks = fai.StandardInformation.AllocationSize.QuadPart / ffssi.PhysicalBytesPerSectorForPerformance;
      }
      if(!!(wanted & metadata_flags::blksize))
      {
        stat.st_blksize = (uint16_t) ffssi.PhysicalBytesPerSectorForPerformance;
      }
      if(!!(wanted & metadata_flags::birthtim))
      {
        stat.st_birthtim = to_timepoint(fai.BasicInformation.CreationTime);
      }
      if(!!(wanted & metadata_flags::sparse))
      {
        stat.st_sparse = !!(fai.BasicInformation.FileAttributes & FILE_ATTRIBUTE_SPARSE_FILE);
      }
      if(!!(wanted & metadata_flags::compressed))
      {
        stat.st_compressed = !!(fai.BasicInformation.FileAttributes & FILE_ATTRIBUTE_COMPRESSED);
      }
    ...
    BOOST_AFIO_HEADERS_ONLY_VIRTUAL_SPEC void *try_mapfile() override final
    {
      if(!mapaddr)
      {
        if(!(flags() & file_flags::Write) && !(flags() & file_flags::Append))
        {
          ...
          {
            auto unsectionh = detail::Undoer([&sectionh]
                                             {
                                               CloseHandle(sectionh);
                                             });
  ...
  inline async_io_handle_windows::async_io_handle_windows(async_file_io_dispatcher_base *_parent, std::shared_ptr<async_io_handle> _dirh, const filesystem::path &path, file_flags flags, bool _SyncOnClose, HANDLE _h)
      : async_io_handle(_parent, std::move(_dirh), path, flags), h(new asio::windows::random_access_handle(_parent->p->pool->io_service(), int_checkHandle(_h, path))), myid(_h), has_been_added(false), SyncOnClose(_SyncOnClose), mapaddr(nullptr)
  {
  ...
  public:
    ...
    static std::pair<NTSTATUS, HANDLE> ntcreatefile(async_path_op_req req) BOOST_NOEXCEPT
    {
      ...
      else if(!!(req.flags & file_flags::Truncate))
        creatdisp |= 0x00000005 /*FILE_OVERWRITE_IF*/;
      else
        creatdisp |= 0x00000001 /*FILE_OPEN*/;
      if(!!(req.flags & file_flags::OSDirect))
        flags |= 0x00000008 /*FILE_NO_INTERMEDIATE_BUFFERING*/;
      if(!!(req.flags & file_flags::AlwaysSync))
        flags |= 0x00000002 /*FILE_WRITE_THROUGH*/;
      ...
      return std::make_pair(NtCreateFile(&h, access, &oa, &isb, &AllocationSize, attribs, fileshare, creatdisp, flags, NULL, 0), h);
    }
  ...
  private:
    ...
    completion_returntype dofile(size_t id, async_io_op, async_path_op_req req)
    {
      ...
      auto ret = std::make_shared<async_io_handle_windows>(this, dirh, req.path, req.flags, (file_flags::SyncOnClose | file_flags::Write) == (req.flags & (file_flags::SyncOnClose | file_flags::Write | file_flags::AlwaysSync)), h);
    ...
    completion_returntype dosymlink(size_t id, async_io_op op, async_path_op_req req)
    {
      ...
      asio::windows::overlapped_ptr ol(threadsource()->io_service(), [ this, id, BOOST_AFIO_LAMBDA_MOVE_CAPTURE(dirop), BOOST_AFIO_LAMBDA_MOVE_CAPTURE(buffer) ](const asio::error_code &ec, size_t bytes)
                                       {
                                         boost_asio_symlink_completion_handler(id, std::move(dirop), std::move(buffer), ec, bytes);
                                       });
      DWORD bytesout = 0;
...
#define FSCTL_SET_ZERO_DATA CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 50, METHOD_BUFFERED, FILE_WRITE_DATA)
    ...
    // Called in unknown thread
    void boost_asio_readwrite_completion_handler(bool is_write, size_t id, std::shared_ptr<async_io_handle> h, std::shared_ptr<std::tuple<atomic<bool>, atomic<size_t>, detail::async_data_op_req_impl<true>>> bytes_to_transfer, std::tuple<off_t, size_t, size_t, size_t> pars, const asio::error_code &ec,
    ...
    template <bool iswrite> void doreadwrite(size_t id, std::shared_ptr<async_io_handle> h, detail::async_data_op_req_impl<iswrite> req, async_io_handle_windows *p)
    {
      ...
      if(!!(p->flags() & file_flags::OSDirect))
      {
        ...
        asio::windows::overlapped_ptr ol(threadsource()->io_service(), [ this, id, h, bytes_to_transfer, BOOST_AFIO_LAMBDA_MOVE_CAPTURE(t) ](const asio::error_code &ec, size_t bytes)
                                         {
                                           boost_asio_readwrite_completion_handler(iswrite, id, std::move(h), bytes_to_transfer, std::move(t), ec, bytes);
                                         });
        ol.get()->Offset = (DWORD)(req.where & 0xffffffff);
        ol.get()->OffsetHigh = (DWORD)((req.where >> 32) & 0xffffffff);
        BOOL ok = iswrite ? WriteFileGather(p->native_handle(), &elems.front(), (DWORD) amount, NULL, ol.get()) : ReadFileScatter(p->native_handle(), &elems.front(), (DWORD) amount, NULL, ol.get());
        DWORD errcode = GetLastError();
        if(!ok && ERROR_IO_PENDING != errcode)
        {
          // std::cerr << "ERROR " << errcode << std::endl;
      ...
      else
      {
        ...
        for(auto &b : req.buffers)
        {
          ...
          DWORD bytesmoved = 0;
          BOOL ok = iswrite ? WriteFile(p->native_handle(), asio::buffer_cast<const void *>(b), (DWORD) asio::buffer_size(b), &bytesmoved, ol.get()) : ReadFile(p->native_handle(), (LPVOID) asio::buffer_cast<const void *>(b), (DWORD) asio::buffer_size(b), &bytesmoved, ol.get());
          DWORD errcode = GetLastError();
          if(!ok && ERROR_IO_PENDING != errcode)
          {
            // std::cerr << "ERROR " << errcode << std::endl;
    ...
    struct enumerate_state : std::enable_shared_from_this<enumerate_state>
    {
      ...
      enumerate_state(size_t _id, async_io_op _op, std::shared_ptr<promise<std::pair<std::vector<directory_entry>, bool>>> _ret, async_enumerate_op_req _req) : id(_id), op(std::move(_op)), ret(std::move(_ret)), req(std::move(_req)) { reallocate_buffer(); }
    ...
    void doenumerate(enumerate_state_t state)
    {
      ...
      do
      {
        ...
        if(IsWow64Process(GetCurrentProcess(), &isWow64), !isWow64)
          ApcContext = nullptr;
#endif
        ntstat = NtQueryDirectoryFile(p->native_handle(), ol.get()->hEvent, NULL, ApcContext, (PIO_STATUS_BLOCK) ol.get(), buffer.get(), (ULONG)(sizeof(FILE_ID_FULL_DIR_INFORMATION) * req.maxitems), FileIdFullDirectoryInformation, FALSE, req.glob.empty() ? NULL : &_glob, req.restart);
    ...
    completion_returntype doenumerate(size_t id, async_io_op op, async_enumerate_op_req req, std::shared_ptr<promise<std::pair<std::vector<directory_entry>, bool>>> ret)
    {
      ...
      enumerate_state_t state = std::make_shared<enumerate_state>(id, std::move(op), std::move(ret), std::move(req));
...
#define FSCTL_QUERY_ALLOCATED_RANGES CTL_CODE(FILE_DEVICE_FILE_SYSTEM, 51, METHOD_NEITHER, FILE_READ_DATA)
    ...
    // Called in unknown thread
    completion_returntype doextents(size_t id, async_io_op op, std::shared_ptr<promise<std::vector<std::pair<off_t, off_t>>>> ret) { return doextents(id, op, std::move(ret), 16); }
    // Called in unknown thread
      ...
      try
      {
        ...
        if(!!(req & fs_metadata_flags::flags) || !!(req & fs_metadata_flags::namemax) || !!(req & fs_metadata_flags::fstypename))
        {
          ...
          if(!!(req & fs_metadata_flags::namemax))
            ret.f_namemax = ffai->MaximumComponentNameLength;
        ...
        if(!!(req & fs_metadata_flags::bsize) || !!(req & fs_metadata_flags::blocks) || !!(req & fs_metadata_flags::bfree) || !!(req & fs_metadata_flags::bavail))
        {
          ...
          if(!!(req & fs_metadata_flags::bsize))
            ret.f_bsize = fffsi->BytesPerSector * fffsi->SectorsPerAllocationUnit;
          if(!!(req & fs_metadata_flags::blocks))
            ret.f_blocks = fffsi->TotalAllocationUnits.QuadPart;
          if(!!(req & fs_metadata_flags::bfree))
            ret.f_bfree = fffsi->ActualAvailableAllocationUnits.QuadPart;
          if(!!(req & fs_metadata_flags::bavail))
            ret.f_bavail = fffsi->CallerAvailableAllocationUnits.QuadPart;
        }
  ...
  struct win_actual_lock_file : public actual_lock_file
  {
    ...
    async_file_io_dispatcher_base::completion_returntype lock(size_t id, async_io_op op, async_lock_op_req req, void *_thislockfile) override final
    {
      ...
      {
        ntstat = NtLockFile(h, thislockfile->ev.native_handle(), nullptr, nullptr, (PIO_STATUS_BLOCK) &ol, &offset, &length, 0, false, req.type == async_lock_op_req::Type::write_lock);
        // printf("L %u ntstat=%u\n", id, ntstat);
      }
      if(STATUS_PENDING != ntstat)
      {
