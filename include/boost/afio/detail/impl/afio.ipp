...
template <class T> using default_engine_unordered_map_spinlock = spinlock<T, spins_to_loop<100>::policy, spins_to_yield<500>::policy, spins_to_sleep::policy>;
template <class Key, class T, class Hash = BOOST_SPINLOCK_V1_NAMESPACE::fnv1a_hash<Key>, class Pred = std::equal_to<Key>, class Alloc = std::allocator<std::pair<const Key, T>>>
using engine_unordered_map_t = BOOST_SPINLOCK_V1_NAMESPACE::concurrent_unordered_map<Key, T, Hash, Pred, Alloc, default_engine_unordered_map_spinlock>;
struct null_lock
{
  void lock() {}
...
namespace detail
{
  ...
  protected:
    actual_lock_file(filesystem::path p) : path(p), lockfilepath(p) { lockfilepath += ".afiolockfile"; }
...
namespace detail
{
  ...
  struct async_io_handle_posix : public async_io_handle
  {
    ...
    async_io_handle_posix(async_file_io_dispatcher_base *_parent, std::shared_ptr<async_io_handle> _dirh, const filesystem::path &path, file_flags flags, bool _DeleteOnClose, bool _SyncOnClose, int _fd)
        : async_io_handle(_parent, std::move(_dirh), path, flags), fd(_fd), has_been_added(false), DeleteOnClose(_DeleteOnClose), SyncOnClose(_SyncOnClose), has_ever_been_fsynced(false), mapaddr(nullptr), mapsize(0)
    {
      if(fd != -999)
    ...
    ~async_io_handle_posix() { int_close(); }
  ...
  struct async_file_io_dispatcher_op
  {
    ...
    void fillStack() {}
#endif
    async_file_io_dispatcher_op(OpType _optype, async_op_flags _flags) : optype(_optype), flags(_flags)
    {
  ...
  struct async_file_io_dispatcher_base_p
  {
    ...
    std::unordered_map<filesystem::path, std::weak_ptr<async_io_handle>, filesystem_hash> dirhcache;

    async_file_io_dispatcher_base_p(std::shared_ptr<thread_source> _pool, file_flags _flagsforce, file_flags _flagsmask) : pool(_pool), flagsforce(_flagsforce), flagsmask(_flagsmask), monotoniccount(0)
    {
#ifdef BOOST_AFIO_USE_CONCURRENT_UNORDERED_MAP
...
#define HAVE_BIRTHTIMESPEC
        | metadata_flags::birthtim | metadata_flags::sparse
      //| metadata_flags::compressed
      ;
#endif
  return ret;
                  ...
                  if(dladdr(addr, &info))
                  {
                    ...
                    if(info.dli_fname)
                    {
                      ...
                      if(ih)
                      {
                        ...
                        std::cerr << info.dli_fname << ":0 (+0x" << std::hex << ((size_t) addr - (size_t) info.dli_fbase) << ")" << std::dec;
                    }
                    else
                      std::cerr << "unknown:0";
                  }
...
namespace detail
{
  class async_file_io_dispatcher_compat : public async_file_io_dispatcher_base
  {
    ...
    completion_returntype dofile(size_t id, async_io_op, async_path_op_req req)
    {
      ...
      auto ret = std::make_shared<async_io_handle_posix>(this, dirh, req.path, req.flags, (file_flags::CreateOnlyIfNotExist | file_flags::DeleteOnClose) == (req.flags & (file_flags::CreateOnlyIfNotExist | file_flags::DeleteOnClose)),
                                                         (file_flags::SyncOnClose | file_flags::Write) == (req.flags & (file_flags::SyncOnClose | file_flags::Write | file_flags::AlwaysSync)), BOOST_AFIO_POSIX_OPEN(req.path.c_str(), flags, 0x1b0 /*660*/));
