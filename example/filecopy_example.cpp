...
namespace
{
      ...
      size_t chunk_size = 1024 * 1024 /* 1Mb */)
  {
    ...
    auto oh = dispatcher->file(
        async_path_op_req(dest, file_flags::Create | file_flags::Write));
    ...
    for(auto &&source : sources)
      ihs_reqs.push_back(async_path_op_req(
          source, file_flags::Read | file_flags::WillBeSequentiallyAccessed));
    ...
    for(auto &ih : ihs)
    {
      ...
      offsets.push_back(std::make_tuple(
          offset, bytes, std::unique_ptr<char[]>(new char[chunk_size])));
      offset += bytes;
    }
    ...
    for(int idx = 0; idx < (int) ihs_reqs.size(); idx++)
    {
      ...
      for(off_t o = 0; o < bytes; o += chunk_size)
      {
        ...
        auto readchunk = dispatcher->read(
            make_async_data_op_req(last, buffer.get(), thischunk, o));
        // Schedule a writing of buffer to offset offset+o after readchunk is
        // ready
        auto writechunk = dispatcher->write(make_async_data_op_req(
            readchunk, buffer.get(), thischunk, offset + o));
    ...
    // Having scheduled all the reads and write, return a future which returns
    // when
    // they're done
...
int main(int argc, const char *argv[])
{
  ...
  try
  {
    ...
    auto h =
        async_concatenate_files(written, totalbytes, dispatcher, dest, sources);
    // Print progress once a second until it's done
    while(future_status::timeout == h.wait_for(boost::afio::chrono::seconds(1)))
    {
      std::cout << "\r" << (100 * written) / totalbytes << "% complete ("
                << written << " out of " << totalbytes << " @ "
                << (written /
