...
#define MULTIPLIER                                                             \
  3900000000ULL  // output number of CPU clocks instead of seconds
...
static std::pair<bool, std::shared_ptr<boost::afio::async_io_handle>>
...
int main(void)
{
  ...
  typedef chrono::duration<double, ratio<1, 1>> secs_type;
  {
    ...
    while(chrono::duration_cast<secs_type>(
              chrono::high_resolution_clock::now() - begin).count() < 1)
    {
    ...
    while(chrono::duration_cast<secs_type>(
              chrono::high_resolution_clock::now() - begin).count() < 2)
    {
      auto now2 = chrono::high_resolution_clock::now();
    ...
    while(chrono::duration_cast<secs_type>(
              chrono::high_resolution_clock::now() - begin).count() < 3)
    {
    ...
    timesliceoverhead =
        time_point::duration(timesliceoverhead.count() / total1);
    std::cout << "Timing overhead is calculated to be "
              << chrono::duration_cast<secs_type>(overhead).count()
              << " seconds." << std::endl;
    std::cout << "OS context switch overhead is calculated to be "
              << chrono::duration_cast<secs_type>(timesliceoverhead).count()
              << " seconds." << std::endl;
    std::cout << "OS sleep overhead is calculated to be "
              << chrono::duration_cast<secs_type>(sleepoverhead).count()
              << " seconds." << std::endl;
  }

  std::pair<async_op_flags, async_file_io_dispatcher_base::completion_t *>
...
#endif
      << std::endl << std::endl;
  csv << "Concurrency,Handler Min,Handler Max,Handler Average,Handler "
         "Stddev,Complete Min,Complete Max,Complete Average,Complete Stddev"
      << std::endl;
  for(size_t concurrency = 0; concurrency < CONCURRENCY; concurrency++)
  {
    ...
    std::cout << "Running " << iterations << " iterations of concurrency "
    ...
    for(size_t n = 0; n < iterations; n++)
    {
      ...
      for(size_t c = 0; c <= concurrency; c++)
      {
        handler[n] = chrono::duration_cast<secs_type>(handled[c] - begin[c] -
                                                      overhead).count();
        complete[n] = chrono::duration_cast<secs_type>(end[c] - handled[c] -
                                                       overhead).count();
      }
    }
    for(size_t n = 0; n <= concurrency; n++)
      if(last[n].id > id_offset)
        id_offset = last[n].id;
    double minHandler = 1 << 30, maxHandler = 0, totalHandler = 0,
           minComplete = 1 << 30, maxComplete = 0, totalComplete = 0;
    for(size_t n = 0; n < iterations; n++)
    {
...
#endif
    csv << concurrency + 1 << "," << minHandler << "," << maxHandler << ","
        << totalHandler << "," << varianceHandler << "," << minComplete << ","
        << maxComplete << "," << totalComplete << "," << varianceComplete
        << std::endl;
  }
  return 0;
}
