...
static std::pair<bool, std::shared_ptr<boost::afio::handle>>
_callback(size_t id, boost::afio::future<> op)
{
...
int main(void)
{
  ...
  typedef chrono::duration<double, ratio<1, 1>> secs_type;
  {
    ...
    std::cout << "Timing overhead is calculated to be "
              << chrono::duration_cast<secs_type>(overhead).count() << " seconds."
              << std::endl;
...
# ifdef BOOST_SMT_PAUSE
        BOOST_SMT_PAUSE
# endif  /*_ defined(BOOST_SMT_PAUSE) _*/
        ;
      waiter.store(false);
      for(auto &i : threads)
        i.join();
      ...
      {
        handler[n] =
        chrono::duration_cast<secs_type>(handled[c] - begin[c] - overhead).count();
        complete[n] =
        chrono::duration_cast<secs_type>(end[c] - handled[c] - overhead).count();
      }
    }
