...
template <class data_store>
void benchmark(const char *filename, const char *desc, bool parallel_writes)
{
...
#pragma omp parallel for
      for(int o = 0; o < todo; o++)
      {
        ...
        *s >> t;
        if(t != items[m + (size_t) o].second)
          std::cerr << "ERROR: Item " << m << " has incorrect contents!"
                    << std::endl;
  ...
  for(auto &i : results)
    csvh << std::get<0>(i) << "," << std::get<1>(i) << "," << std::get<2>(i)
         << std::endl;
}
