...
struct idirectstream : public std::istream
{
  ...
  idirectstream(afio::handle_ptr h, std::shared_ptr<file_buffer_type> buffer, size_t length) : std::istream(new directstreambuf(std::move(h), std::move(buffer), length)), buf(static_cast<directstreambuf *>(rdbuf()))
  {
  }
