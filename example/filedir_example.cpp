...
int main(void)
{
  ...
  try
  {
    ...
    boost::afio::async_path_op_req(dispatcher->close(mklink), "testdir/linktodir")));
    // Schedule deleting the file only after when it has been created
    auto rmfile(dispatcher->rmfile(
    boost::afio::async_path_op_req(dispatcher->close(mkfile), "testdir/testfile")));
    ...
    auto rmdir(
    dispatcher->rmdir(boost::afio::async_path_op_req(barrier.front(), "testdir")));
