...
int main(void)
{
  ...
  try
  {
    ...
    auto mkdir(dispatcher->dir(boost::afio::async_path_op_req(
        "testdir", boost::afio::file_flags::Create)));
    // Schedule creating a file called testfile in testdir only when testdir has
    // been created
    auto mkfile(dispatcher->file(boost::afio::async_path_op_req(
        mkdir, "testdir/testfile", boost::afio::file_flags::Create)));
    // Schedule creating a symbolic link called linktodir to the item referred
    // to by the precondition
    // i.e. testdir. Note that on Windows you can only symbolic link
    // directories.
    auto mklink(dispatcher->symlink(boost::afio::async_path_op_req(
    ...
    auto rmdir(dispatcher->rmdir(
        boost::afio::async_path_op_req(barrier.front(), "testdir")));
    // Check ops for errors
    boost::afio::when_all({mkdir, mkfile, mklink, rmlink, rmfile, rmdir})
        .wait();
  }
  catch(...)
  {
