...
callback(size_t, boost::afio::async_io_op op)
{
#if 0
...
int main(void)
{
  ...
  callbacks(1, std::make_pair(async_op_flags::none, callback));
#if 0
...
#pragma omp parallel for
  ...
  std::cout << "It took " << diff.count()
            << " secs to execute 5,000,000 closures which is "
            << (5000000 / diff.count()) << " unchained closures/sec" << std::endl;
