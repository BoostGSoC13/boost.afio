...
The search took 2.5173 seconds which was 14592.6 files per second or 2663.89
Mb/sec.

Cold cache:
91 files matched out of 36422 files which was 6108537728 bytes.
The search took 388.74 seconds which was 93.6925 files per second or 14.9857
Mb/sec.

Warm cache mmaps:
92 files matched out of 36734 files which was 7031400686 bytes.
The search took 1.02974 seconds which was 35673.1 files per second or 6512.01
Mb/sec.
...
public:
  ...
  void dosearch(std::shared_ptr<async_io_handle> h, const char *buffer,
                size_t length)
  {
...
#pragma omp critical
  ...
  file_read(size_t id, async_io_op op,
            std::shared_ptr<std::vector<
                char, detail::aligned_allocator<char, 4096, false>>> _buffer,
            size_t length)
  {
    ...
    docompleted(2);
    // Throw away the buffer now rather than later to keep memory consumption
    // down
    _buffer->clear();
...
// std::cout << "F " << h->path() << std::endl;
...
#endif
    {
      ...
      auto buffer = std::make_shared<
          std::vector<char, detail::aligned_allocator<char, 4096, false>>>(
          _length + 1);
      ...
      auto read_done = dispatcher->completion(
          read, std::make_pair(
                    async_op_flags::none /*regex search might be slow*/,
                    std::function<async_file_io_dispatcher_base::completion_t>(
                        std::bind(&find_in_files::file_read, this,
  ...
  // completes
  std::pair<bool, std::shared_ptr<async_io_handle>> dir_enumerated(
      size_t id, async_io_op op,
      std::shared_ptr<future<std::pair<std::vector<directory_entry>, bool>>>
          listing)
  {
    std::shared_ptr<async_io_handle> h(op.get());
...
// For each of the directories schedule an open and enumeration
    ...
    // concurrency
    // when used with OSDirect so throttle directory enumerations to enforce
    // some depth first traversal.
    {
      std::pair<async_op_flags,
                std::function<async_file_io_dispatcher_base::completion_t>>
          dir_openedf = std::make_pair(async_op_flags::none,
                                       std::bind(&find_in_files::dir_opened,
...
#endif
        {
          auto dir_open = dispatcher->dir(
              async_path_op_req(lastdir, h->path() / entry.name()));
...
// For each of the files schedule an open and search
...
#endif
        {
          ...
          if(length)
          {
            ...
            auto file_open = dispatcher->file(
                async_path_op_req(lastdir, h->path() / entry.name(), flags));
  ...
  // A directory enumerating completion, called once per directory open in the
  // tree
  ...
  find_in_files(const char *_regexpr)
      : regexpr(_regexpr),
        ...
        dispatcher(make_async_file_io_dispatcher(
            process_threadpool(),
            file_flags::WillBeSequentiallyAccessed /*|file_flags::OSDirect*/)),
...
int main(int argc, const char *argv[])
{
  ...
  if(argc < 2)
  {
    std::cerr << "ERROR: Specify a regular expression to search all files in "
