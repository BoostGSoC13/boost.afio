...
#pragma omp critical
  ...
  std::pair<bool, std::shared_ptr<async_io_handle>> file_read(
...
// std::cout << "F " << h->path() << std::endl;
...
#endif
    {
      ...
      std::vector<char, detail::aligned_allocator<char, 4096, false>>>(_length + 1);
...
// For each of the directories schedule an open and enumeration
    ...
    // depth first traversal.
    {
      ...
      for(auto &entry : entries)
      {
        ...
        {
          auto dir_open =
          dispatcher->dir(async_path_op_req(lastdir, h->path() / entry.name()));
...
// For each of the files schedule an open and search
      ...
      for(auto &entry : entries)
      {
...
#endif
        {
          ...
          if(length)
          {
            ...
            async_op_flags::none,
            std::function<async_file_io_dispatcher_base::completion_t>(
            std::bind(&find_in_files::file_opened, this, std::placeholders::_1,
      ...
      , completed(0)
  {
    ...
    std::make_pair(async_op_flags::none,
                   std::function<async_file_io_dispatcher_base::completion_t>(
                   std::bind(&find_in_files::dir_opened, this, std::placeholders::_1,
                             std::placeholders::_2))));
