...
/* My Intel Core i7 3770K running Windows 8 x64 with 7200rpm
drive, using
Sysinternals RAMMap to clear disc cache
(http://technet.microsoft.com/en-us/sysinternals/ff700229.aspx)

Warm cache:
92 files matched out of 36734 files which was 7031545071
bytes.
The search took 2.5173 seconds which was 14592.6 files per
second or 2663.89 Mb/sec.

Cold cache:
91 files matched out of 36422 files which was 6108537728
bytes.
The search took 388.74 seconds which was 93.6925 files per
second or 14.9857 Mb/sec.

Warm cache mmaps:
92 files matched out of 36734 files which was 7031400686
bytes.
The search took 1.02974 seconds which was 35673.1 files per
second or 6512.01 Mb/sec.

Cold cache mmaps:
91 files matched out of 36422 files which was 6109258426
bytes.
The search took 242.76 seconds which was 150.033 files per
second or 24 Mb/sec.
*/
...
// Often it's easiest for a lot of nesting callbacks to
// carry state via a this pointer
class find_in_files
{
public:
  ...
  std::atomic<size_t> bytesread, filesread, filesmatched,
      scheduled, completed;
  std::vector<std::pair<boost::afio::filesystem::path,
                        size_t>> filepaths;
  ...
  void dosearch(std::shared_ptr<async_io_handle> h,
...
#pragma omp critical
  ...
  // A file searching completion, called when each file read
  // completes
  std::pair<bool, std::shared_ptr<async_io_handle>>
  file_read(size_t id, async_io_op op,
            std::shared_ptr<std::vector<
                char, detail::aligned_allocator<
                          char, 4096, false>>> _buffer,
            size_t length)
  {
    ...
    docompleted(2);
    // Throw away the buffer now rather than later to keep
    // memory consumption down
    _buffer->clear();
    return std::make_pair(true, h);
  }
  // A file reading completion, called when each file open
  // completes
  std::pair<bool, std::shared_ptr<async_io_handle>>
  file_opened(size_t id, async_io_op op, size_t length)
  {
    std::shared_ptr<async_io_handle> h(op.get());
// std::cout << "F " << h->path() << std::endl;
...
#endif
    {
      ...
      auto buffer = std::make_shared<std::vector<
          char,
          detail::aligned_allocator<char, 4096, false>>>(
          _length + 1);
      ...
      auto read_done = dispatcher->completion(
          ...
          std::make_pair(
              async_op_flags::
                  none /*regex search might be slow*/,
              std::function<async_file_io_dispatcher_base::
                                completion_t>(std::bind(
                  &find_in_files::file_read, this,
                  std::placeholders::_1,
  ...
  // An enumeration parsing completion, called when each
  // directory enumeration completes
  std::pair<bool, std::shared_ptr<async_io_handle>>
  dir_enumerated(
      size_t id, async_io_op op,
      std::shared_ptr<future<std::pair<
          std::vector<directory_entry>, bool>>> listing)
  {
    std::shared_ptr<async_io_handle> h(op.get());
    async_io_op lastdir,
        thisop(dispatcher->op_from_scheduled_id(id));
    // Get the entries from the ready future
    std::vector<directory_entry> entries(
        std::move(listing->get().first));
// std::cout << "E " << h->path() << std::endl;
// For each of the directories schedule an open and
// enumeration
#if 0
        // Algorithm 1
        {
            ...
            if(!dir_reqs.empty())
            {
                std::vector<std::pair<async_op_flags, std::function<async_file_io_dispatcher_base::completion_t>>> dir_openedfs(dir_reqs.size(), std::make_pair(async_op_flags::None, std::bind(&find_in_files::dir_opened, this, std::placeholders::_1, std::placeholders::_2)));
...
#else
    // Algorithm 2
    // The Windows NT kernel filing system driver gets upset
    // with too much concurrency
    // when used with OSDirect so throttle directory
    // enumerations to enforce some depth first traversal.
    {
      std::pair<
          async_op_flags,
          std::function<
      ...
      for(auto &entry : entries)
      {
        if(entry.st_type() ==
           ...
           boost::afio::filesystem::file_type::
               directory_file)
#else
           boost::afio::filesystem::file_type::directory)
#endif
        {
          auto dir_open = dispatcher->dir(async_path_op_req(
              lastdir, h->path() / entry.name()));
          auto dir_opened =
              dispatcher->completion(dir_open, dir_openedf);
...
// For each of the files schedule an open and search
                ...
                std::make_pair(
                    ...
                    std::function<
                        async_file_io_dispatcher_base::
                            completion_t>(std::bind(
                        &find_in_files::file_opened, this,
                        std::placeholders::_1,
  ...
  // A directory enumerating completion, called once per
  // directory open in the tree
  std::pair<bool, std::shared_ptr<async_io_handle>>
  dir_opened(size_t id, async_io_op op)
  {
    std::shared_ptr<async_io_handle> h(op.get());
    // std::cout << "D " << h->path() << std::endl;
    // Now we have an open directory handle, schedule an
    // enumeration
    auto enumeration =
        dispatcher->enumerate(async_enumerate_op_req(
            dispatcher->op_from_scheduled_id(id),
            metadata_flags::size, 1000));
    auto listing = std::make_shared<future<
        std::pair<std::vector<directory_entry>, bool>>>(
        std::move(enumeration.first));
  ...
  find_in_files(const char *_regexpr)
      : regexpr(_regexpr),
        // Create an AFIO dispatcher that bypasses any
        // filing system buffers
        dispatcher(make_async_file_io_dispatcher(
            process_threadpool(),
            file_flags::
                WillBeSequentiallyAccessed /*|file_flags::OSDirect*/)),
        bytesread(0), filesread(0), filesmatched(0),
        scheduled(0), completed(0)
  {
    filepaths.reserve(50000);

    // Schedule the recursive enumeration of the current
    // directory
    std::cout << "\n\nStarting directory enumerations ..."
              << std::endl;
...
int main(int argc, const char *argv[])
{
  ...
  if(argc < 2)
  {
    std::cerr << "ERROR: Specify a regular expression to "
                 "search all files in the current "
                 "directory." << std::endl;
    return 1;
  }
  // Prime SpeedStep
  auto begin = chrono::high_resolution_clock::now();
  while(chrono::duration_cast<secs_type>(
  ...
  try
  {
    ...
    std::cout << "\n" << finder.filesmatched
              << " files matched out of "
              << finder.filesread << " files which was "
              << finder.bytesread << " bytes." << std::endl;
    std::cout << "The search took " << diff.count()
              << " seconds which was "
              << finder.filesread / diff.count()
              << " files per second or "
              << (finder.bytesread / diff.count() / 1024 /
                  1024) << " Mb/sec." << std::endl;
  }
  catch(...)
  {
    std::cerr
