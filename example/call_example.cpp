...
int main(void)
{
  ...
  // Schedule an asynchronous call of some function with some bound set of
  // arguments
  ...
  // Schedule as asynchronous call of some function to occur only after
  // helloworld completes
  auto addtovalue =
      dispatcher->call(helloworld.second,
                       [](boost::afio::shared_future<int> v) -> int
                       {
                         // v is highly likely to be ready very soon by the time
                         // we are called
                         return v.get() + 1;
                       },
                       helloworld.first);
