...
The search took 8.32834 seconds which was 4716.3 files per second or 812.318
Mb/sec.

Single threaded, cold cache:
91 files matched out of 38967 files which was 6170927489 bytes.
The search took 369.046 seconds which was 105.588 files per second or 15.9467
Mb/sec.

OpenMP, warm cache:
92 files matched out of 38943 files which was 7092655881 bytes.
The search took 3.73611 seconds which was 10423.4 files per second or 1810.46
Mb/sec.

OpenMP, cold cache:
91 files matched out of 38886 files which was 6170656567 bytes.
The search took 741.131 seconds which was 52.4684 files per second or 7.94029
Mb/sec.
*/
...
int main(int argc, const char *argv[])
{
  ...
  if(argc < 2)
  {
    cerr << "ERROR: Specify a regular expression to search all files in the "
...
#pragma omp parallel for schedule(dynamic)
    for(int n = 0; n < (int) filepaths.size(); n++)
    {
      ...
      s.seekg(0, ios::beg);
      // Allocate a sufficient buffer, avoiding the byte clearing vector would
      // do
