...
int main(void)
{
  ...
  //
  // What we're going to do is this: for each item in groups, schedule
  // item.first
  ...
  // preceding group's barrier completes. Repeat until all groups have been
  // executed.
  ...
  // This lambda is what each parallel op in each group will do: increment an
  // atomic
  // for that group.
  ...
  // This lambda is called after each barrier completes, and it checks that
  // exactly
  ...
  for(auto &run : groups)
  {
    ...
    // barrier() is very easy: its number of output ops exactly matches its
    // input
    // but none of the output will complete until the last of the input
    // completes
    auto thisgroupbarriered = dispatcher->barrier(thisgroupcallops);
    // Schedule a call of the verify lambda once barrier completes. Here we
    // choose
    // the first item of the barrier's return, but in truth any of them are
    // good.
