...
int main(void)
{
  ...
  try
  {
    ...
    boost::afio::async_io_op resizedfile(dispatcher->truncate(
        openfile, 12)); /*< schedules resize file ready for writing after open
                           file completes >*/
    ...
    boost::afio::async_io_op written(
        dispatcher->write(boost::afio::make_async_data_op_req(
            resizedfile, buffers,
            0))); /*< schedules write after resize file completes >*/
    ...
    // Schedule the closing and deleting of example_file.txt after the contents
    // read
    req.precondition = dispatcher->close(
        read); /*< schedules close file after read completes >*/
    boost::afio::async_io_op deletedfile(dispatcher->rmfile(
        req)); /*< schedules delete file after close completes >*/
    ...
    boost::afio::when_all({openfile, resizedfile, written, stored, read})
        .wait(); /*< waits for file open, resize, write, sync and read to
                    complete, throwing any exceptions encountered >*/
    ...
    boost::afio::when_all({req.precondition /*close*/, deletedfile})
        .wait(); /*< waits for file close and delete to complete, throwing any
