...
int main(void)
{
  try
  {
    ...
    .get(); /*< waits for file open, resize, write, sync and read to complete,
  ...
  {
    std::cerr << "ERROR: program exits via system_error code " << e.code().value()
              << " (" << e.what() << ")" << std::endl;
    return 1;
  }
  catch(const std::exception &e)
  {
    std::cerr << "ERROR: program exits via exception (" << e.what() << ")"
              << std::endl;
    return 1;
  }
  catch(...)
  {
    std::cerr << "ERROR: program exits via "
