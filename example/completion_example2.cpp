...
int main(void)
{
  ...
  // One thing direct programming of completion handlers can do which call()
  // cannot is immediate
  // completions. These run immediately after the precondition finishes by the
  // thread worker
  // which executed the precondition rather than being appended to the FIFO
  // queue. This can
      ...
      -> std::pair<bool, std::shared_ptr<boost::afio::async_io_handle>>
  {
    ...
    // Create some callable entity which will do the actual completion. It can
    // be
    // anything you like, but you need a minimum of its integer id.
    auto completer =
        [](std::shared_ptr<boost::afio::async_file_io_dispatcher_base>
               dispatcher,
           size_t id, boost::afio::async_io_op op) -> int
    {
      try
      {
        ...
        // Note that op.get() rethrows any exception in op. Normally you want
        // this.
        dispatcher->complete_async_op(id, op.get());
      }
      catch(...)
      {
        // In non-deferred completions AFIO traps exceptions for you. Here, you
        // must
  ...
  std::function<boost::afio::async_file_io_dispatcher_base::completion_t>
      boundf = std::bind(completion, dispatcher,
  ...
  boost::afio::async_io_op op = dispatcher->completion(
      ...
      std::make_pair(
          ...
          completion() returns) */
          boost::afio::async_op_flags::immediate, boundf));

  // Create a boost::future<> representing the ops passed to when_all()
  boost::afio::future<
      std::vector<std::shared_ptr<boost::afio::async_io_handle>>> future =
