...
int main(void)
{
  ...
  // One thing direct programming of completion handlers can do which call() cannot
  // is immediate
  // completions. These run immediately after the precondition finishes by the thread
  // worker
  // which executed the precondition rather than being appended to the FIFO queue.
  // This can
  ...
  // boost::afio::dispatcher::completion_t
  ...
  boost::afio::future<> op = dispatcher->completion(
  boost::afio::future<>() /* no precondition */,
  std::make_pair(
  ...
  boost::afio::stl_future<std::vector<std::shared_ptr<boost::afio::handle>>>
  stl_future = boost::afio::when_all_p(op);
