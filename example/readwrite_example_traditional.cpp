...
int main(void)
{
  ...
  try
  {
    ...
    std::fstream openfile(
        "example_file.txt"); /*< opens file >*/
    // Turn on exception throwing
    openfile.exceptions(std::fstream::failbit |
                        std::fstream::badbit);

    // Do a write gather. STL iostreams will buffer the
    // writes
    ...
    // Make sure the previous batch has definitely reached
    // physical storage
    ...
    openfile.read(buffer.data(),
                  buffer.size()); /*< reads >*/

    // Close the file and delete it
    openfile.close(); /*< closes file >*/
    boost::afio::filesystem::remove(
        "example_file.txt"); /*< deletes file >*/
