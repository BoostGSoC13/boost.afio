...
int main(void)
{
...

  {
    ...
    auto dir_opened =
    dispatcher->dir(async_path_op_req(""));  // "" means current directory in AFIO
    auto file_enumed = dispatcher->enumerate(
    async_enumerate_op_req(dir_opened, metadata_flags::size, 2, true, "foo.txt"));
    ...
    auto file_read =
    dispatcher->read(make_async_data_op_req(file_opened, (void *) input,
