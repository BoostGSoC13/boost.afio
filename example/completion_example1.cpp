...
int main(void)
{
  ...
  // therefore the least
  // overhead but at the cost of considerable extra programmer effort. You
      ...
      -> std::pair<bool, std::shared_ptr<boost::afio::async_io_handle>>
  {
    /* id is the unique, non-zero integer id of this op.
       precondition is the op you supplied as precondition. As it will by
       definition
       have completed by now, you can fetch from its h member variable a shared
       pointer
       to the shared future containing either the output async_io_handle or the
       error state.
    */
  ...
  std::function<boost::afio::async_file_io_dispatcher_base::completion_t>
      boundf =
          std::bind(completer,
                    ...
                    /* Any values for the user defined parameters. Remember
                       ALWAYS to pass by value! */
                    std::string("Hello world"));

  // Schedule an asynchronous call of the completion with some bound set of
  // arguments
  boost::afio::async_io_op helloworld = dispatcher->completion(
