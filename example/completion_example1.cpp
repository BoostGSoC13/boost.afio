...
int main(void)
{
                   ...
                   -> std::pair<bool, std::shared_ptr<boost::afio::handle>>
  {
    /* id is the unique, non-zero integer id of this op.
       ...
       pointer
       to the shared stl_future containing either the output handle or the error
       state.
    */
  ...
  std::function<boost::afio::dispatcher::completion_t> boundf = std::bind(
  completer,
  ...
  stl_future = boost::afio::when_all_p(helloworld);
