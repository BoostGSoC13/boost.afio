...
For 2 concurrent writers, achieved 1720.7 attempts per second with a success rate of
1335.24 writes per second which is a 77.5984% success rate.

Benchmarking file locks via atomic append with 2 concurrent writers ...
Waiting for threads to exit ...
For 2 concurrent writers, achieved 2413.66 attempts per second with a success rate of
790.364 writes per second which is a 32.7455% success rate.
Traditional locks were 1.6894 times faster.

Without file_flags::TemporaryFile | file_flags::DeleteOnClose, traditional file locks
were:

Benchmarking traditional file locks with 2 concurrent writers ...
Waiting for threads to exit ...
For 2 concurrent writers, achieved 1266.4 attempts per second with a success rate of
809.013 writes per second which is a 63.883% success rate.
...
int main(int argc, const char *argv[])
{
    ...
    writers = totalwriters = atoi(argv[1]);
  {
    ...
    auto mkdir1(
        dispatcher->dir(async_path_op_req(mkdir, "testdir/1", file_flags::Create)));
    auto mkdir2(
        dispatcher->dir(async_path_op_req(mkdir, "testdir/2", file_flags::Create)));
    auto mkdir3(
        dispatcher->dir(async_path_op_req(mkdir, "testdir/3", file_flags::Create)));
    auto mkdir4(
        dispatcher->dir(async_path_op_req(mkdir, "testdir/4", file_flags::Create)));
    auto mkdir5(
        dispatcher->dir(async_path_op_req(mkdir, "testdir/5", file_flags::Create)));
    auto mkdir6(
        dispatcher->dir(async_path_op_req(mkdir, "testdir/6", file_flags::Create)));
    auto mkdir7(
        dispatcher->dir(async_path_op_req(mkdir, "testdir/7", file_flags::Create)));
    auto mkdir8(
        dispatcher->dir(async_path_op_req(mkdir, "testdir/8", file_flags::Create)));
      ...
      threads.push_back(thread(
          [&done, &attempts, &successes, n]
          {
            try
            {
              ...
              while(!done)
              {
                // Traditional file locks are very simple: try to exclusively create
                // the lock file.
          ...
          [&done, &attempts, &successes, n]
          {
            try
            {
              ...
              while(!done)
              {
                ...
                auto writetolog(dispatcher->write(make_async_data_op_req(
                    extendlog, {logentry, mythreadid, logentryend}, where)));
                // Fetch errors from the last operation first to avoid sleep-wake
                // cycling
                writetolog.get();
                extendlog.get();
                successes.fetch_add(1, memory_order_relaxed);
          ...
          [&done, &attempts, &successes, thread]
          {
            try
            {
              ...
              auto lockfilez(dispatcher->file(async_path_op_req(
                  "testdir/log.lock", file_flags::Create | file_flags::ReadWrite)));
...
#pragma pack(pop)
                ...
                temp.uniqueid = thread;  // TODO FIXME: Needs to be a VERY random
                                         // number to prevent collision.
                dispatcher->write(make_async_data_op_req(lockfilea, temp.bytes,
                                                         sizeof(temp), 0)).get();

                // Step 2: Wait until my interest message appears, also figure out
                // what interests precede
                ...
                auto iterate = [&]
                {
                      ...
                      buffersoffset -= sizeof(buffers))
                  {
                    ...
                    dispatcher->read(make_async_data_op_req(lockfilez,
                                                            (void *) buffers, amount,
                                                            buffersoffset)).get();
                ...
                // Step 3: If there is no lock and no interest precedes mine, claim
                // the mutex. Else issue a nominate for myself,
                  ...
                  for(;;)
                  {
                    ...
                    if(preceding.empty())
                    {
                      ...
                      if(startofinterest < myuniqueid)
                      {
                        ...
                        //                    std::cout << thread << ": lock taken
                        //                    for myuniqueid=" << myuniqueid << ",
                        //                    zeroing " << range.front().first << ",
