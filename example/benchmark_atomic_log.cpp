...
For 2 concurrent writers, achieved 1720.7 attempts per second with a success
rate of 1335.24 writes per second which is a 77.5984% success rate.

Benchmarking file locks via atomic append with 2 concurrent writers ...
Waiting for threads to exit ...
For 2 concurrent writers, achieved 2413.66 attempts per second with a success
rate of 790.364 writes per second which is a 32.7455% success rate.
Traditional locks were 1.6894 times faster.

Without file_flags::TemporaryFile | file_flags::DeleteOnClose, traditional file
locks were:

Benchmarking traditional file locks with 2 concurrent writers ...
Waiting for threads to exit ...
For 2 concurrent writers, achieved 1266.4 attempts per second with a success
rate of 809.013 writes per second which is a 63.883% success rate.
...
int main(int argc, const char *argv[])
{
    ...
    writers = totalwriters = atoi(argv[1]);
  {
    ...
    auto mkdir(
        dispatcher->dir(async_path_op_req("testdir", file_flags::Create)));
    auto mkdir1(dispatcher->dir(
        async_path_op_req(mkdir, "testdir/1", file_flags::Create)));
    auto mkdir2(dispatcher->dir(
        async_path_op_req(mkdir, "testdir/2", file_flags::Create)));
    auto mkdir3(dispatcher->dir(
        async_path_op_req(mkdir, "testdir/3", file_flags::Create)));
    auto mkdir4(dispatcher->dir(
        async_path_op_req(mkdir, "testdir/4", file_flags::Create)));
    auto mkdir5(dispatcher->dir(
        async_path_op_req(mkdir, "testdir/5", file_flags::Create)));
    auto mkdir6(dispatcher->dir(
        async_path_op_req(mkdir, "testdir/6", file_flags::Create)));
    auto mkdir7(dispatcher->dir(
        async_path_op_req(mkdir, "testdir/7", file_flags::Create)));
    auto mkdir8(dispatcher->dir(
        async_path_op_req(mkdir, "testdir/8", file_flags::Create)));
...
#undef PRINT_FIELD
  ...
  if(1)
  {
    std::cout << "\nBenchmarking a single traditional lock file with "
      ...
      threads.push_back(thread(
          [&done, &attempts, &successes, n]
          {
            try
            {
              ...
              auto logfile(dispatcher->file(async_path_op_req(
                  "testdir/log", file_flags::Create | file_flags::ReadWrite)));
              ...
              while(!done)
              {
                // Traditional file locks are very simple: try to exclusively
                // create the lock file.
                // If you succeed, you have the lock.
                auto lockfile(dispatcher->file(async_path_op_req(
                    "testdir/log.lock", file_flags::CreateOnlyIfNotExist |
                                            file_flags::Write |
                                            file_flags::TemporaryFile |
                                            file_flags::DeleteOnClose)));
                attempts.fetch_add(1, memory_order_relaxed);
                // v1.4 of the AFIO engine will return error_code instead of
                // exceptions for this
                try
                {
                  lockfile.get();
                ...
                std::string logentry("I am log writer "),
                    mythreadid(to_string(n)), logentryend("!\n");
                // Fetch the size
                off_t where = logfile->lstat().st_size,
                      entrysize = logentry.size() + mythreadid.size() +
                                  logentryend.size();
                // Schedule extending the log file
                auto extendlog(
                    dispatcher->truncate(logfile, where + entrysize));
                // Schedule writing the new entry
                auto writetolog(dispatcher->write(make_async_data_op_req(
                    extendlog, {logentry, mythreadid, logentryend}, where)));
                writetolog.get();
                extendlog.get();
            ...
            {
              std::cerr << "ERROR: test exits via system_error code "
                        << e.code().value() << "(" << e.what() << ")"
                        << std::endl;
              abort();
            }
            ...
            {
              std::cerr << "ERROR: test exits via unknown exception"
                        << std::endl;
              abort();
            }
          }));
    ...
    std::cout << "For " << writers << " concurrent writers, achieved "
              ...
              << (100.0 * successes / attempts) << "% success rate."
              << std::endl;
              ...
              while(!done)
              {
                ...
                lockfiles.push_back(async_path_op_req(
                    ...
                    file_flags::CreateOnlyIfNotExist | file_flags::Write |
                        file_flags::TemporaryFile | file_flags::DeleteOnClose));
                lockfiles.push_back(async_path_op_req(
                    "testdir/2/log.lock",
                    file_flags::CreateOnlyIfNotExist | file_flags::Write |
                        file_flags::TemporaryFile | file_flags::DeleteOnClose));
                ...
                auto writetolog(dispatcher->write(make_async_data_op_req(
                    extendlog, {logentry, mythreadid, logentryend}, where)));
                // Fetch errors from the last operation first to avoid
                // sleep-wake cycling
                writetolog.get();
                extendlog.get();
                successes.fetch_add(1, memory_order_relaxed);
      ...
      threads.push_back(thread(
          [&done, &attempts, &successes, n]
          {
            try
            {
              ...
              {
                attempts.fetch_add(1, memory_order_relaxed);
                // **** WARNING UNSUPPORTED UNDOCUMENTED API DO NOT USE IN YOUR
                // CODE ****
      ...
      threads.push_back(std::thread(
          [&done, &attempts, &successes, thread]
          {
            try
            {
              ...
              auto lockfilez(dispatcher->file(async_path_op_req(
                  "testdir/log.lock",
                  file_flags::Create | file_flags::ReadWrite)));
              // Schedule opening the lock file for atomic appending
              auto lockfilea(dispatcher->file(async_path_op_req(
                  "testdir/log.lock", file_flags::Create | file_flags::Write |
                                          file_flags::Append)));
              // Retrieve any errors which occurred
              lockfilea.get();
              lockfilez.get();
...
#pragma pack(pop)
                ...
                // TODO FIXME: If multiple machines are all accessing the lock
                // file, nowtimestamp
                ...
                temp.uniqueid = thread;  // TODO FIXME: Needs to be a VERY
                                         // random number to prevent collision.
                dispatcher->write(make_async_data_op_req(lockfilea, temp.bytes,
                                                         sizeof(temp), 0))
                    .get();

                // Step 2: Wait until my interest message appears, also figure
                // out what interests precede
                //         mine and where my start of interest begins, and if
                //         someone currently has the lock
                off_t startofinterest =
                    dispatcher->extents(lockfilez).first.get().front().first;
                ...
                auto iterate = [&]
                {
                  ...
                  off_t lockfilesize =
                      lockfilez->lstat(metadata_flags::size).st_size;
                  buffersoffset = lockfilesize > sizeof(buffers) ?
                                      lockfilesize - sizeof(buffers) :
                                      0;
                  // buffersoffset-=buffersoffset % sizeof(buffers[0]);
                  for(; !validPrecedingCount &&
                            buffersoffset >= startofinterest &&
                      ...
                      buffersoffset -= sizeof(buffers))
                  {
                    ...
                    dispatcher->read(make_async_data_op_req(
                                         lockfilez, (void *) buffers, amount,
                                         buffersoffset)).get();
                    for(size_t n = amount / sizeof(buffers[0]) - 1;
                        !validPrecedingCount && n < amount / sizeof(buffers[0]);
                        n--)
                    {
                      ...
                      {
                        startofinterest =
                            buffersoffset + n * sizeof(buffers[0]);
                        break;
                      }
                                   ...
                                   myuniqueid))
                      {
                          ...
                          if(buffers[n].uniqueid < myuniqueid &&
                             preceding.end() ==
                                 std::find(
                                     ...
                                     std::make_pair(
                                         false, (off_t) buffers[n].uniqueid)))
                          {
                            preceding.push_back(
                        ...
                        {
                          if(buffersoffset + n * sizeof(buffers[0]) <
                                 myuniqueid &&
                             preceding.end() ==
                                 std::find(
                                     preceding.begin(), preceding.end(),
                ...
                std::cout << std::endl;
                  ...
                  if(findPreceding)
                  {
                    ...
                    preceding.erase(
                        std::remove_if(preceding.begin(), preceding.end(),
                ...
                do
                {
                  ...
                  iterate();
                  // Didn't find it, so sleep and retry, maybe st_size will have
                  // updated by then
                  if(myuniqueid == (off_t) -1)
                    this_thread::sleep_for(chrono::milliseconds(1));
                } while(myuniqueid == (off_t) -1);

                // Step 3: If there is no lock and no interest precedes mine,
                // claim the mutex. Else issue a nominate for myself,
                  ...
                  for(;;)
                  {
                    ...
                    if(preceding.empty())
                    {
                      ...
                      dispatcher->write(make_async_data_op_req(
                                            lockfilea, temp.bytes, sizeof(temp),
                                            0)).get();
                    ...
                    else
                    {
                      ...
                      // TODO FIXME: Put a modify watch on the lockfile instead
                      // of spinning
                      ...
                      // c.wait_for(l, chrono::milliseconds(1), [&fileChanged]{
                ...
                std::string logentry("I am log writer "),
                    mythreadid(to_string(thread)), logentryend("!\n");
                // Fetch the size
                off_t where = logfile->lstat().st_size,
                      entrysize = logentry.size() + mythreadid.size() +
                                  logentryend.size();
                ...
                extendlog.get();
                successes.fetch_add(1, memory_order_relaxed);
                //              std::cout << thread << ": doing work for
                //              myuniqueid=" << myuniqueid << std::endl;
                ...
                temp.timestamp = gettime();
                temp.uniqueid = myuniqueid;
                //              std::cout << thread << ": lock released for
                //              myuniqueid=" << myuniqueid << std::endl;
                dispatcher->write(make_async_data_op_req(lockfilea, temp.bytes,
                                                         sizeof(temp), 0))
                    .get();
  ...
  std::cout << "Traditional locks were "
            << (traditional_locks / atomic_log_locks) << " times faster."
            << std::endl;
  return 0;
}
//]
