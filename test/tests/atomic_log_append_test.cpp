    ...
    threads.push_back(thread([&done, n]
                             {
                               try
                               {
                                 ...
                                 auto logfilez(dispatcher->file(async_path_op_req("testdir/log", file_flags::Create | file_flags::ReadWrite)));
                                 // Schedule opening the log file for atomic appending of log entries
                                 auto logfilea(dispatcher->file(async_path_op_req("testdir/log", file_flags::Create | file_flags::Write | file_flags::Append)));
                                 ...
                                 while(!done)
                                 {
                                   ...
                                   stat_t s = dispatcher->write(make_async_data_op_req(logfilea, buffer.bytes, 32, 0))->lstat();
                                   if(s.st_allocated > 8192 || s.st_size > 8192)
                                   {
                                     ...
                                     std::vector<std::pair<off_t, off_t>> extents = dispatcher->extents(logfilez).first.get();
                               ...
                               catch(const system_error &e)
                               {
                                 ...
                                 BOOST_FAIL("Unit test exits via exception");
                                 abort();
                               }
