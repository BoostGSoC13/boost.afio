...
BOOST_AFIO_AUTO_TEST_CASE(async_io_lstat_works, "Tests that async i/o lstat() works", 60)
{
    ...
    auto mkfile(dispatcher->file(async_path_op_req::relative(mkdir, "file", file_flags::Create)));
    auto mklink(dispatcher->symlink(async_path_op_req::absolute(mkdir, "testdir/linktodir", file_flags::Create)));
    ...
    auto rmlink(dispatcher->rmsymlink(mklink));
    auto rmfile(dispatcher->rmfile(dispatcher->depends(rmlink, mkfile)));
    auto rmdir(dispatcher->rmdir(dispatcher->depends(rmfile, mkdir)));
    auto rmtest(dispatcher->rmdir(dispatcher->depends(rmdir, test)));
    when_all({rmlink, rmfile, rmdir, rmtest}).wait();
}
