...
#define BOOST_AFIO_CHECK_SYSTEM_ERROR_CODE(call, errcode) try { call; BOOST_FAIL("Exception not thrown by " #call); } catch(const system_error &e) { BOOST_CHECK(e.code().value()==errcode); } catch(...) { BOOST_FAIL("Exception thrown by " #call " was not a system_error"); }
#ifdef WIN32
...
# define BOOST_AFIO_BAD_FD EBADF
    ...
    // Create a bad handle
    async_io_op op=dispatcher->rmfile(async_path_op_req(dispatcher->close(dispatcher->file(async_path_op_req("testfile", file_flags::Create))), "testfile"));
    char buffer[32];
