...
#else  /*_ BOOST_AFIO_USE_BOOST_THREAD && defined(BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY) _*/
  std::vector<int> allresults;
  for(auto &i : results)
    allresults.push_back(i.get());
#endif  /*_ BOOST_AFIO_USE_BOOST_THREAD && defined(BOOST_THREAD_PROVIDES_FUTURE_WHEN_ALL_WHEN_ANY) _*/
