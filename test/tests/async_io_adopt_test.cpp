...
{
  test_handle(async_file_io_dispatcher_base *parent) : async_io_handle(parent, std::shared_ptr<async_io_handle>(), "foo", file_flags::None) {}
  virtual void close()
  {
    // Do nothing
  }
  virtual void *native_handle() const { return nullptr; }
  virtual directory_entry direntry(metadata_flags wanted = directory_entry::metadata_fastpath()) const { return directory_entry(); }
  virtual filesystem::path target() const { return filesystem::path(); }
