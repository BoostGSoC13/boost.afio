...
#ifdef __FreeBSD__
    static BOOST_CONSTEXPR_OR_CONST size_t ITEMS=10;  // ZFS really, really, really hates this test
#else
...
//[race_protection_example
    try
    {
...

      {
        ...
        std::cout << "Creating 100 directories ..." << std::endl;
        when_all(dirs).get();
        dirh=dirs.front();
        atomic<bool> done(false);
        std::cout << "Creating worker thread to constantly rename those 100 directories ..." << std::endl;
        ...
        {
          if(!newfiles.empty())
            std::cout << "Iteration " << i << ": Renaming 100 files and directories inside the 100 constantly changing directories ..." << std::endl;
          for(size_t n=0; n<ITEMS; n++)
          {
            if(!newfiles.empty())
            {
          ...
          std::cout << "Iteration " << i << ": Creating 100 files and directories inside the 100 constantly changing directories ..." << std::endl;
